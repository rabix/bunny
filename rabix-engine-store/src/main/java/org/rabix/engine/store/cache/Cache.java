...
public class Cache {
  ...
  public synchronized void put(Cachable cachable, CacheItem.Action action) {
    if (cachable == null) {
      return;
    }
    ...
    if (cache.containsKey(key)) {
      ...
      if (item.action != CacheItem.Action.INSERT && action != CacheItem.Action.NOOP) {
        item.action = action;
      }
    } else {
  ...
  public synchronized void putIfAbsent(Cachable cachable, CacheItem.Action action) {
    if (cachable == null) {
      return;
    }
    CacheKey key = cachable.getCacheKey();
  ...
  public synchronized <T extends Cachable> List<T> merge(List<T> cachables, Class<T> clazz) {
    ...
    for (Cachable cachable : cachables) {
      ...
      if (!CollectionUtils.isEmpty(cached)) {
        ...
        put(cachable, CacheItem.Action.NOOP);
