...
public interface JDBIVariableRecordRepository extends VariableRecordRepository {
  ...
  @SqlUpdate("insert into variable_record (job_id,value,port_id,type,link_merge,is_wrapped,globals_count,times_updated_count,context_id,is_default,transform) values (:job_id,:value,:port_id,:type,:link_merge,:is_wrapped,:globals_count,:times_updated_count,:root_id,:is_default,:transform)")
  int insert(@BindVariableRecord VariableRecord jobRecord);
  
  @SqlUpdate("update variable_record set value=:value,link_merge=:link_merge,is_wrapped=:is_wrapped,globals_count=:globals_count,times_updated_count=:times_updated_count,is_default=:is_default,transform=:transform where port_id=:port_id and root_id=:root_id and job_id=:job_id and type=:type")
  ...
  VariableRecord get(@Bind("job_id") String jobId, @Bind("port_id") String portId, @Bind("type") LinkPortType type, @Bind("root_id") String rootId);
