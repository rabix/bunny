...
import org.rabix.tes.download.LocalDownloadServiceImpl;
import org.rabix.tes.service.TESStorageService;
import org.rabix.tes.service.impl.TESExecutorServiceImpl;
import org.rabix.tes.service.impl.TESStorageServiceImpl;
...
public class TESBackendCommandLine {

  private static final Logger logger = LoggerFactory.getLogger(TESBackendCommandLine.class);
  ...
  private static File getConfigDir(CommandLine commandLine, Options options) throws IOException {
    ...
    File config = new File(new File(TESBackendCommandLine.class.getProtectionDomain().getCodeSource().getLocation().getPath()).getParentFile().getParentFile() + "/config");
